 ...
 * {@code provision/{uuid}/response}, publishes an empty body to
 * {@code provision/{uuid}/request}, and returns the
 * {@link ProvisionResponse} (iotDataEndpoint + iotCredentialsEndpoint +
 * provisioningToken).
 *
 * <p>Replaces the legacy {@code MgmtCloudRouter} that round-tripped on
 * {@code endpoint/v1/{uuid}} against mgmt-services.</p>
 */
public class ProvisioningRouter {
     ...
     * <ul>
     *   <li>{@link ProvisionErrorCode#NOT_CLAIMED} — the subscription
     *       is left open and the future stays incomplete. claim_reactor
     *       pushes a success payload to the same topic when the
     *       DynamoDB claim transition fires. The caller's outer
     *       wall-clock timeout (~15 min) is the safety net for a
     *       missed push.</li>
     *   <li>Any other code ({@code DECOMMISSIONED}, {@code INTERNAL},
     *       {@code UNKNOWN}) — the future is completed exceptionally
     *       so the caller can tear down and retry, since the same
     *       error will just repeat on the same subscription.</li>
     * </ul>
     */
    public Future<ProvisionResponse> route(String uuid)
            throws InterruptedException, RetryableProvisioningException {
        return route(uuid, AWS_IOT_DEFAULT_TIMEOUT_SECONDS);
    }

    /**
     * Same as {@link #route(String)} but with an overridable
     * subscription-ack timeout.
     *
     * @param timeout subscription-ack timeout in seconds (applied to
     *                the MQTT SUBACK / PUBACK only; the future itself
     *                never times out from here — the caller owns the
     *                wall clock).
     */
    public Future<ProvisionResponse> route(String uuid, int timeout)
            throws InterruptedException, RetryableProvisioningException {
        ...
        CompletableFuture<Integer> subscribed = client.subscribeToProvisionResponse(
                ...
                future::complete,
                (errorResponse) -> {
                    ProvisionErrorCode code = errorResponse.code != null
                            ? errorResponse.code
                            : ProvisionErrorCode.UNKNOWN;
                    String msg = errorResponse.message == null ? "" : errorResponse.message;
                    if (code == ProvisionErrorCode.NOT_CLAIMED) {
                        // claim_reactor will push a success payload to
                        // the same topic when the DynamoDB claim
                        // transition fires. Stay subscribed; the
                        // caller's wall-clock timeout is the safety
                        // net for a missed push.
                        logger.atInfo().log(
                                "Router responded NotClaimed: {}. Subscription stays open; "
                ...
                (e) -> {
                    // Protocol-level deserialisation failure is a real
                    // bug (router and device disagree on the wire
                    // format). Surface it so the outer loop can drop
                    // and re-establish.
        ...
        // Empty MQTT body. The Router lambda reads uuid from topic(2);
        // the claim-cert IoT policy enforces the topic-to-clientId
        // binding so the publisher's identity is already authenticated.
